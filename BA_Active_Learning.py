...
def choose_first_samples(influences):
    # waehlt zufaellig 20 Konfigurationen aus dem Trainingsset
...
def loop(influences, test_data, booleans, perf_names):
    ...
    for i in range(30):
        if i == 0:
            ...
            new_model, new_f_names, all_f_names = train_model(new_model, booleans2, perfs, coefs_mean, coefs_std, influences.columns)
        ...
        influences, new_samples, coefs_mean, coefs_std = choose_next_samples(influences, coefs, all_f_names)
...
def train_model(lmodel, start_sample_bools, start_sample_perfs, coefs_mean, coefs_std, colnames):
    ...
    lmodel = lmodel.fit(X, start_sample_perfs)
    # print("lmodel coef")
    # print(lmodel.coef_)
...
def choose_next_samples(influences, coefs, combined_feature_names):
    ...
    for i in range(10):
        idx = np.where(coefs_std == np.amax(coefs_std))[0][:5]
        # print(f"influences.columns: {influences.columns}")
        idx2 = [] # Liste mit Listen, jede SubListe enthaelt eine FeatureKombo oder ein einzelnes Feature
        for i in idx:
            tmpi = combined_feature_names[i].split(" ")
            idx2.append(tmpi)
        # print(f"new idx: {idx2}")
        # print(f"np.amax(coefs_std) = {np.amax(coefs_std)}")
        # print(f"std-abw in choose next sample: {coefs_std}")
        new_configs, influences = find_according_konfigs(influences, idx2)
        tmpinfls.append(new_configs)
    return influences, tmpinfls, coefs_mean, coefs_std

def find_according_konfigs(influences, index):
    tmp = influences
    tmp0 = influences # fuer den Fall, dass nur Offset groesste Ungenauigkeit hat wird aus allen Trainingsdaten
    tmp1 = influences #      zufaellig gewaehlt
    for i in index:
        for word in i:
            if word == '1': # word='1' => gibt es in orig. Daten nicht, da dies der Offset ist TODO: Offset?!
                continue
            tmp0 = tmp[tmp[word] == 0]
            tmp1 = tmp[tmp[word] == 1]
    candidates1 = list(set(tmp1.index.values))
    candidates0 = list(set(tmp0.index.values))
    new_configs = []
    for i in range(20):
        if len(candidates0) == 0 and len(candidates1) == 0:
            break
        ...
        if len(candidates0) > 0:
            new_config = np.random.choice(candidates0)
            new_configs.append(influences.loc[new_config])
